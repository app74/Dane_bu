...
test.describe("bez prihlásenia Microsoft Entra ID", () => {
  test("stránka ponúkne prihlásenie a API nevydá dáta", async ({ page, request }) => {
    await page.goto("/");
    await expect(page.getByRole("link", { name: "Prihlásiť sa účtom Microsoft" })).toBeVisible();
...
test.describe("prihlásený používateľ", () => {
  ...
  test("hlavný scenár: subjekt, náhľad, QR, história a trvalosť dát", async ({ page }) => {
    const errors: string[] = [];
    page.on("pageerror", (e) => errors.push(e.message));
    await page.goto("/");
    await expect(page.getByText("Prihlásený: e2e.tester@example.com")).toBeVisible();
    ...
    await expect(page.getByRole("status").filter({ hasText: "Subjekt bol uložený." })).toBeVisible();

    await page.getByLabel("Potvrdené pravidlo").selectOption("income-return-2025");
    await page.getByLabel("Suma v EUR").fill("100,00");
    await page.getByRole("button", { name: "Vytvoriť náhľad" }).click();
    await expect(page.getByRole("heading", { name: "Kontrolný náhľad" })).toBeVisible();
    ...
    await expect(page.getByRole("button", { name: "Uložené v histórii" })).toBeVisible();

    await page.reload();
    await expect(page.getByRole("heading", { name: "História" })).toBeVisible();
    await expect(page.getByText(/VS 1700992025/)).toBeVisible();
    await expect(page.getByLabel("Vybraný subjekt")).toContainText("Azure E2E subjekt");
    expect(errors).toEqual([]);
  });

  test("vyhľadanie FS vypnuté v teste vráti hlásenie o ručnom zadaní", async ({ request }) => {
